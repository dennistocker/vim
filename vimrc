...
" Use Vim settings, rather than Vi setttings
set nocompatible
...
if has("cscope")
    ...
    nmap <F8>f :vert scs find f <C-R>=expand("<cfile>")<CR><CR>
...
" comments
" -------------------
" # backward search current word == /currentword
" * forward  ...
"
" g** commands
" gd --- jump to local var definition.
" ga --- get ASCII code of current character.
" gf --- open the file named of current string.
" gD --- same with gd, can search global variable.
" guw -- change to lower case.
" gUw -- change to upper case.
" g~w -- change to lower/upper case.
...
nnoremap <F12> :YcmForceCompileAndDiagnostics<CR>	"force recomile with syntastic
nmap <F8> :YcmDiags<CR>
nnoremap <F9> :lne<CR>
nnoremap <F10> :lp<CR>

nnoremap gl :YcmCompleter GoToImprecise<CR>
